...
import type { BloodGroup, BloodRequest, Urgency } from "../types";
...
export default function HospitalDashboard() {
  ...
  const [selectedReq, setSelectedReq] = useState<BloodRequest | null>(
    requests[0],
  );
  const [showCreate, setShowCreate] = useState(false);
  const [showNotif, setShowNotif] = useState(false);
  const [notifications, setNotifications] = useState(mockNotifications);
  const [activeTab, setActiveTab] = useState<"timeline" | "donors" | "network">(
    "timeline",
  );
      ...
      <div className="glass border-b border-bb-border shrink-0">
        <div className="flex overflow-x-auto divide-x divide-bb-border">
          {[
            ...
            {
              value: requests.reduce((s, r) => s + r.unitsRequired, 0),
                ...
                {req.bloodBankUnits > 0 && (
                  <div className="mt-2 progress-bar">
                    <div
                      ...
                      style={{
                        width: `${(req.bloodBankUnits / req.unitsRequired) * 100}%`,
                      }}
                  ...
                  <div className="rounded-xl border border-bb-border bg-bb-surface p-3">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <span className="font-mono text-sm font-bold text-bb-teal">
                        {selectedReq.bloodBankUnits}/{selectedReq.unitsRequired}
                      </span>
                    </div>
                    <div className="progress-bar">
                      <div
                        className="progress-fill bg-bb-teal"
                        style={{
                          width: `${selectedReq.unitsRequired > 0 ? (selectedReq.bloodBankUnits / selectedReq.unitsRequired) * 100 : 0}%`,
                        }}
                      />
                    </div>
                  </div>
                  <div className="rounded-xl border border-bb-border bg-bb-surface p-3">
                    ...
                    <div className="progress-bar">
                      <div
                        ...
                        style={{
                          width:
                            selectedReq.donorUnits > 0
                              ? `${(selectedReq.donorUnits / selectedReq.unitsRequired) * 100}%`
                              : "0%",
                        }}
                      />
                    </div>
                ...
                {(["timeline", "donors", "network"] as const).map((tab) => (
                  ...
                  >
                    {tab === "timeline"
                      ? "Journey"
                      : tab === "donors"
                        ? `Donors (${selectedReq.donors?.length ?? 0})`
                        : "Network View"}
                  </button>
                ))}
              </div>
                    ...
                    {(selectedReq.donors?.length ?? 0) === 0 ? (
                      <div className="text-center py-10">
                        <p className="text-bb-muted text-sm">
                          No donor matches yet for this request.
                        </p>
                      </div>
                    ) : (
                      <>
                  ...
                  <div className="flex items-start gap-2.5">
                    <span
                      ...
                      style={{
                        ...
                        animationPlayState: n.read ? "paused" : "running",
                      }}
                      aria-hidden="true"
                    />
                    <div>
