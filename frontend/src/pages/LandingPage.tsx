...
const steps = [
  { icon: '🏥', label: 'VERIFY', desc: 'Hospital confirms patient blood requirement', color: '#38bdf8' },
  { icon: '🏦', label: 'INVENTORY SCAN', desc: 'System searches nearby blood-bank inventory', color: '#00bfb3' },
  { icon: '🤖', label: 'SMART MATCHING', desc: 'AI engine ranks eligible donors by compatibility', color: '#818cf8' },
  { icon: '✓', label: 'FULFILLMENT', desc: 'Fastest path delivers verified blood units', color: '#10b981' },
];
  ...
  return (
    <div className="min-h-screen bb-network-bg text-bb-text overflow-x-hidden">
      ...
      <nav className="fixed top-0 inset-x-0 z-40 glass border-b border-bb-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <a href="#how" className="text-sm text-bb-muted hover:text-bb-text transition-colors hidden sm:block">How it works</a>
            <button
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-2 lg:grid-cols-4 gap-4">
          {stats.map((s) => (
            <div key={s.label} className="text-center">
              <p className="font-mono text-2xl sm:text-3xl font-bold" style={{ color: s.color }}>{s.value}</p>
      ...
      <section id="how" className="py-20 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-14">
            <p className="font-mono text-xs uppercase tracking-widest text-bb-muted mb-3">The Workflow</p>
            <h2 className="text-3xl sm:text-4xl font-bold text-bb-text">From Request to Fulfillment</h2>
            <p className="text-bb-dim mt-3 max-w-xl mx-auto">Four intelligent stages ensure the right blood reaches the right patient at the right time.</p>
          </div>

          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {steps.map((s, i) => (
              <div key={s.label} className="glass rounded-2xl p-5 relative overflow-hidden group hover:scale-[1.02] transition-transform duration-300">
                <div className="absolute top-0 right-0 font-mono text-7xl font-black opacity-5 text-bb-text leading-none pr-4 pt-1" aria-hidden="true">
                  {i + 1}
                </div>
                <div
                ...
                <p className="font-mono text-xs font-bold tracking-widest mb-2" style={{ color: s.color }}>{s.label}</p>
                <p className="text-sm text-bb-dim leading-snug">{s.desc}</p>
                {i < steps.length - 1 && (
                  <div className="hidden lg:block absolute -right-3 top-1/2 -translate-y-1/2 z-10 text-bb-muted" aria-hidden="true">→</div>
                )}
              </div>
            ))}
          </div>
      ...
      <section className="py-20 px-4 sm:px-6 border-t border-bb-border">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-14">
            <h2 className="text-3xl font-bold text-bb-text">Three Nodes. One Network.</h2>
            <p className="text-bb-dim mt-3">Each role is a trusted, verified participant in the BloodBridge network.</p>
          </div>
          <div className="grid sm:grid-cols-3 gap-4 max-w-4xl mx-auto">
            {[
              { icon: '🏥', title: 'Hospital', desc: 'Create verified blood requests and coordinate fulfillment in real time.', color: '#38bdf8', role: 'hospital' as const },
              { icon: '🏦', title: 'Blood Bank', desc: 'Manage inventory and respond instantly to verified hospital requirements.', color: '#00bfb3', role: 'blood-bank' as const },
              { icon: '🩸', title: 'Donor', desc: 'Join the verified donor network and respond when your blood can save a life.', color: '#818cf8', role: 'donor' as const },
            ].map(r => (
              <button
                key={r.title}
                onClick={() => onNavigate('role-select')}
              ...
              >
                <div className="size-14 rounded-2xl flex items-center justify-center text-2xl mb-4 transition-transform group-hover:scale-110 duration-200" style={{ background: `${r.color}15`, border: `1px solid ${r.color}35` }}>
                  {r.icon}
                </div>
                <h3 className="font-bold text-bb-text text-lg mb-2">{r.title}</h3>
      ...
      <footer className="border-t border-bb-border py-8 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-bb-text">Blood<span className="text-bb-crimson-bright">Bridge</span></span>
          </div>
          <p className="text-xs text-bb-muted font-mono">Connecting verified need with life-saving supply.</p>
        </div>
      </footer>
    </div>
  );
