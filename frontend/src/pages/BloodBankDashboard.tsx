...
export default function BloodBankDashboard() {
  ...
  const [inventory, setInventory] = useState<Record<BloodGroup, number>>(
    bloodBankInventory as Record<BloodGroup, number>,
  );
      ...
      <div className="glass border-b border-bb-border shrink-0">
        <div className="flex overflow-x-auto divide-x divide-bb-border">
          {[
            ...
            {
              value: Object.values(inventory).filter((v) => v === 0).length,
              label: "Out of Stock",
              color: "#c01832",
            },
            {
              value: Object.values(inventory).filter((v) => v > 0 && v <= 2)
                .length,
          ...
          <section>
            ...
            <div className="glass rounded-2xl p-4">
              <InventoryMatrix
                inventory={inventory}
                onGroupClick={(g) =>
                  setHighlightGroup(highlightGroup === g ? undefined : g)
                }
          ...
          <section>
            ...
            {requests.length === 0 ? (
              ...
              <div className="space-y-3">
                {requests.map((req) => {
                  const canFulfill =
                    inventory[req.bloodGroup] >= req.unitsRequired;
                  const offerUnits = Math.min(
                    inventory[req.bloodGroup],
                    req.unitsRequired,
                  );
                  return (
                    <div
                    ...
                    >
                      <div className="flex flex-wrap items-start gap-4 justify-between mb-4">
                        ...
                        <div className="text-right">
                          ...
                          <p
                            className={`font-mono font-bold text-xl ${inventory[req.bloodGroup] === 0 ? "text-bb-crimson-bright" : inventory[req.bloodGroup] < req.unitsRequired ? "text-bb-amber" : "text-bb-teal"}`}
                          >
                      ...
                      <div className="flex gap-2">
                        {offerUnits > 0 ? (
                          <button
                            onClick={() =>
                              handleOffer(req.id, offerUnits, req.bloodGroup)
                            }
